...
import { db, collection, addDoc, setDoc,doc, getDoc, serverTimestamp } from "../firebase";
...
const questions = [
  { id: 1, question: "1. Define Depth First Search?", marks: 2 },
  { id: 2, question: "2. Define NodeS in Graph?", marks: 2 },
  { id: 3, question: "3. Define Edges in Graph?", marks: 2 },
  { id: 4, question: "4. Explain about Directed Graph?", marks: 2 },
  { id: 5, question: "5. Explain AVL Tree", marks: 5 },
...
const ExamPage = () => {
  ...
  const rollNo = String(location.state?.rollNo);
...
useEffect(() => {
    const handleVisibilityChange = () => {
        if (document.hidden) {
            const warningMessage = "Tab switching is not allowed! Your exam session may be terminated.";
            
            toast.warning(warningMessage, { position: "top-center", autoClose: 5000 });

           
            speakMessage(warningMessage);
        }
    };
  ...
  const saveAnswerToFirebase = async (rollNo,questionId, questionText, answerText, marks) => {
    ...
    try {
      ...
      // Reference to the 'exam_answers' subcollection
      const examAnswersRef = collection(studentRef, "exam_answers");
  
      // Add the answer document with auto-generated ID
      await addDoc(examAnswersRef, {
        questionId,
      ...
      console.log("Answer saved successfully in subcollection.");
    } catch (error) {
      console.error("Error saving answer:", error);
      console.log("Error saving anwser", error);
      await speakText(`Error saving answer ${error}`);
    }
    // try {
  ...
  const giveInstructions = async () => {
    ...
    speakText("All The Best For Your Exam");
  ...
  const handleQuestionFlow = async () => {
    ...
    await saveAnswerToFirebase(rollNo,currentQuestion.id, currentQuestion.question, recordedAnswer, currentQuestion.marks);
